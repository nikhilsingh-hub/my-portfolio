...
function BasicInfo({ aboutData }) {
    ...
    return (
        <section ref={basicInfoRef} className='relative rounded-xl mx-4 sm:mx-6 md:mx-14 my-14 font-playpen overflow-hidden bg-gradient-to-br from-[#1A1A1A] to-[#0D0D0D] border border-orange-400/20 shadow-2xl'>
            ...
            <div className="relative z-10 flex flex-col lg:flex-row justify-between space-fluid-lg">
                <div className='lg:w-1/2 space-fluid-md'>
                    <div className="flex flex-col">
                        {/* Main intro section */}
                        <div className="intro space-fluid-md mb-10">
                            <div className="space-y-8">
                                ...
                                <div className="mb-8">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <div>
                                            <h1 className="text-white font-bold text-fluid-3xl leading-tight mb-2">
                                                Hello,<br />
                                    ...
                                    <div className="ml-7 space-y-3 [&>*]:space-y-1 sm:[&>*]:space-y-0">
                                        {intro.map((ele, index) => {
                                            let objectKey = Object.keys(ele)[0];
                                            return (
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-0 sm:gap-2 mb-1 sm:mb-3" key={index}>
                                                    <span className="text-gray-300 text-sm sm:text-2xl font-sniglet leading-tight sm:leading-normal">{objectKey.replace('_', ' ')}:</span>
                                                    <span className="text-orange-400 text-sm sm:text-2xl font-sniglet font-medium leading-tight sm:leading-normal">{ele[objectKey]}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                        ...
                        <div className='flex items-center gap-4'>
                            <a href={personalInfo.resumeLink}
                                className='group relative bg-gradient-to-r from-orange-400 to-orange-600 hover:from-orange-500 hover:to-orange-700 p-4 md:p-5 rounded-xl flex gap-3 items-center font-comic text-fluid-lg text-white font-medium transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-orange-400/30 overflow-hidden'
                                target="_blank" rel='noreferrer'>
                                {/* Button shine effect */}
                                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-700"></div>
                                
                                <span className="relative z-10">{basicInfoData.resumeButton.text}</span>
                                <img src={download} alt={basicInfoData.resumeButton.altText} className='relative z-10 h-5 w-5 md:h-6 md:w-6 group-hover:rotate-12 transition-transform duration-300' />
                            </a>
                            
                            <div className="flex items-center gap-2 text-gray-400 text-fluid-sm">
